...
import { members, categories, competitiveLevels } from '@/app/aster/data';
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        ...
        >
          ...
          {categories.map(c => (
            <option key={c.name} value={c.name}>{c.name} ({c.ageRange})</option>
          ))}
        </select>
        <select
          value={filterGroup}
      ...
      <div className="mb-6 flex gap-4 text-sm text-gray-400">
        <span>Prikazano: <strong className="text-white">{filtered.length}</strong> članica</span>
          ...
          <div key={groupName}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {groupMembers.map(m => (
                <div
                  key={m.id}
                  className="bg-gray-800/40 hover:bg-gray-800/70 rounded-xl p-3.5 transition-all border border-gray-800 hover:border-gray-700 flex items-center gap-3"
                >
                  {/* Avatar */}
                  <div
                    className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
                    style={{
                      background: `${getCategoryColor(m.category)}20`,
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-200 truncate">{m.fullName}</div>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {m.level && (
                        <span
                          ...
                          style={{
                            background: `${getLevelColor(m.level)}15`,
